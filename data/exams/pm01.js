...
const pm01DigitalShiftNormativeAnchors = [
  {
    ...
    sourceEvidence: [
      {
        label: "Реквизиты",
        evidence: "Официальная карточка ФГОС указывает приказ Минобрнауки России от 09.12.2016 N 1569 и редакцию от 17.12.2020.",
        examUse: "Использовать как основную правовую рамку PM01."
      },
      {
        label: "ОК 09/ОК 10",
        evidence: "В требованиях к результатам указаны ОК 09 про ИТ в профессиональной деятельности и ОК 10 про профессиональную документацию.",
        examUse: "Цифровой журнал, cockpit, маркировка и работа с технологической информацией методически допустимы."
      },
      {
        label: "ПК 1.1-1.4",
        evidence: "Вид деятельности 3.4.1 охватывает рабочее место, оборудование, сырье, обработку овощей, рыбы, мяса, птицы, дичи, кролика и полуфабрикаты.",
        examUse: "Пять цехов PM01 должны проверять именно подготовку сырья, полуфабрикаты, качество, безопасность и хранение."
      }
    ]
  },
  {
    ...
    sourceEvidence: [
      {
        label: "Реквизиты",
        evidence: "Официальная карточка ФГОС указывает приказ Минобрнауки России от 09.12.2016 N 1565 и редакцию от 17.12.2020.",
        examUse: "Не заменяет PM01 43.01.09, но даёт расширенную методическую рамку."
      },
      {
        label: "Сложный ассортимент",
        evidence: "Основной вид деятельности описывает организацию и ведение процессов приготовления и подготовки к реализации полуфабрикатов сложного ассортимента.",
        examUse: "Поддерживает тренировочный комплексный заказ, производственные статусы и межцеховой маршрут."
      },
      {
        label: "ОК 09/ОК 10",
        evidence: "ОК 09 и ОК 10 также закрепляют ИТ и профессиональную документацию.",
        examUse: "Обосновывает современные цифровые элементы как тренировочную надстройку, а не изменение официальной ведомости."
      }
    ]
  },
  {
    ...
    sourceEvidence: [
      {
        label: "Реестр",
        evidence: "Карточка ИРПО/ФИРПО показывает ID ПОП СПО 2245 для 43.01.09 Повар, кондитер.",
        examUse: "Использовать как внешний ориентир при сверке локальной РП."
      },
      {
        label: "Статус",
        evidence: "В реестре указаны статус ПОП 'Утверждено', дата размещения 28.02.2022 и номер регистрации 33.",
        examUse: "Фиксирует, что примерная программа существует, но финальные темы всё равно берутся из локальных документов."
      }
    ]
  },
  {
    ...
    sourceEvidence: [
      {
        label: "Граница правок",
        evidence: "ФГОС и ПОП задают рамку компетенций, но локальные темы, часы, формулировки и фонд оценочных средств находятся в РП/КТП.",
        examUse: "Официальные вопросы не переписывать до получения РП/КТП и подтверждения преподавателя."
      },
      {
        label: "Что запросить",
        evidence: "Нужны рабочая программа PM01/МДК, календарно-тематическое планирование и действующие оценочные материалы.",
        examUse: "После получения документов заполняется методическая матрица и только затем готовятся финальные вопросы/assets."
      }
    ]
  }
];

const pm01DigitalShiftNormativeDossier = {
  title: "Нормативное досье PM01",
  verifiedAt: "2026-06-22",
  sourceBasis: "ФГОС 43.01.09, ФГОС 43.02.15, реестр ПОП СПО 43.01.09 ИРПО/ФИРПО и pending-gate локальных РП/КТП.",
  officialScope:
    "Официальный экзамен остается PM01 по 43.01.09: 100 баллов, 20 заданий, 5 вариантов. PX и cockpit используются только как тренировочная надстройка.",
  methodicalBoundary:
    "ФГОС подтверждает компетенции и виды деятельности, но не заменяет рабочую программу преподавателя. Финальные темы, формулировки вопросов и критерии нельзя переписывать до получения РП/КТП.",
  allowedModernization: [
    "цифровой производственный журнал как проявление ОК 09 и ОК 10",
    "интерактивные производственные ситуации по ПК 1.1-1.4",
    "визуальный контроль качества, маркировки, хранения и санитарных решений",
    "тренировочные practice-only симуляторы без влияния на ведомость"
  ],
  blockedUntilRp: [
    "финальное переименование тем по цехам",
    "замена официальных вопросов и правильных ответов",
    "подключение generated final assets",
    "изменение критериев официальной оценки"
  ],
  rpIntakeNeeded: [
    "рабочая программа PM01/МДК 01.01-01.02",
    "КТП с темами и часами",
    "локальные оценочные материалы и требования преподавателя",
    "подтверждение ОК 09/ОК 10 в РП/КТП"
  ]
};
...
module.exports.digitalShift = {
  ...
  normativeDossier: pm01DigitalShiftNormativeDossier,
