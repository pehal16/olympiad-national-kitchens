...
- Release 1.7.0 is not yet recorded here as deployed. Production remains on the last verified deployment until migration, staging load test, UI QA, commit/push, and deployment confirmation are complete.
...
- The local, not-yet-deployed 2026-09-24 landing-screen pass gives the olympiad a quieter college-branded hero, one state-aware primary action, rules/registration links, a three-step explanation, a decorative generated food image under `public/assets/olympiad/landing/`, and a visible initial-load failure/retry state. Registration, start and result navigation now reflect the actual state, form submission and start are protected from duplicate clicks, and early-finish results show the number of answered questions instead of claiming all five tours were completed. The same-tab session pointer now restores a token-protected active attempt or receipt after reload; when browser storage is unavailable, recovery may still require re-registration or organizer help. The landing image is decorative only, not an exam answer reference. Local browser QA passed desktop/mobile landing, registration, active-attempt reload, early finish, result reload, 390px overflow, and zero console errors; Node tests passed 132/132, Cloudflare build and local PM01 verifier passed. The 10,000-seed variant audit reported 10,000 distinct route signatures and no score mismatches, but four T4 questions still appeared in every route and exposure in other tours varied, so teacher calibration and a frozen balanced official set remain necessary. The final revision still needs a full release rehearsal before deployment. `docs/olympiad-launch-plan.md` now records a source-backed fairness/accessibility review and replaces the old assumption of a closed student roster with the user's open-participation decision. Do not infer that open self-registration verifies identity or that browser focus/screenshot signals prove cheating.
