...
- Branch: `main`
- Functional baseline: use the latest committed `origin/main`; historical commit references below are not the current revision.
...
- Visual package `public/assets/olympiad/visual-v1/` contains six individually inspected dish images, fifteen transparent ingredient cards and `asset-review.json` with generation/review records and recipe sources. The participant homepage keeps only a small 3D-demo link in the hero; the former six-dish gallery and three-step guide were removed from the homepage. `/visual-demo.html` remains the no-score hands-on preview for Margherita and the explicitly named Russian restaurant Caesar variant. Real exam 3D tasks are unchanged.
...
- Release 1.7.0 was published as a clearly labelled test/pilot version on Cloudflare Pages from commit `a4810e5` in successful GitHub Actions run `35978344399` on 2026-09-24. The follow-up mobile home-navigation fix is commit `85b287c`, deployed successfully in run `35978914487`; that is the final published code revision for this pass. The workflow applied additive D1 migrations `0006` and `0007` and configured the independent `ATTEMPT_ID_SECRET` without exposing its value. Production `/api/health`, the public olympiad API, landing image, visual demo, and the read-only PM01 regression check returned successfully. The live browser showed desktop/mobile landing, correct mobile home return, working landing CTA, interactive visual demo, and zero console errors; no production participant was created by this QA. This publication is not authorization for official results: teacher approval, balanced/frozen variants, rate limiting, short start window, identity process, and a real RF-network/60-client rehearsal remain open.
- On 2026-09-24 the large six-dish/3D showcase was removed from the participant homepage in commit `fc829fe`; a small `3D-демо` link under the three-step guide still opens the no-score preview. The existing 3D exam questions and their assets were not changed. Cloudflare deployment run `35980299758` succeeded. Production browser QA confirmed the new link, no homepage gallery, desktop/mobile layout without horizontal overflow, and zero console errors; the read-only PM01 production verifier passed. No production participant was created.
...
- The 2026-09-24 certificate pass simplifies the participant home again: the three-step guide and route preview are removed, registration follows the hero, full rules are collapsed until requested, and the small `3D-демо` link sits in the hero. The exam questions and start/answer flow are unchanged. After completion, the participant result page links to the independent `/certificate.html` document rather than embedding a responsive certificate. Its landscape A4 composition is fixed at every screen width; on narrow screens the preview scrolls instead of reflowing. The document fetches the saved attempt using its existing private device token, fills student name, total score, optional mentor, completion date, and attempt reference, and uses the existing `brand-gkts-shield.jpg` crest plus a decorative frame. The print action opens the browser's save-as-PDF dialog. Winner diplomas are separate and not auto-awarded. `showParticipantScore: true` now exposes scores immediately for all completed attempts, including prior completed attempts, per the user's choice. Pilot certificates remain visibly labelled `Тестовый запуск · неофициальный результат`; official wording and result policy still require organizer approval.
