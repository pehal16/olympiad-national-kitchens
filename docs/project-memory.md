...
- Visual package `public/assets/olympiad/visual-v1/` contains six individually inspected dish images, fifteen transparent ingredient cards and `asset-review.json` with generation/review records and recipe sources. The participant homepage keeps only a small 3D-demo link under the three-step guide; the former six-dish gallery was removed from the homepage. `/visual-demo.html` remains the no-score hands-on preview for Margherita and the explicitly named Russian restaurant Caesar variant. Real exam 3D tasks are unchanged.
